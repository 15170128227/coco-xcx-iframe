...
import {api, msg}  from './utils/api'
import {login, appInfo}  from './utils/service'
...
App({
  ...
  globalData: {
    ...
    loginSession: null,
    swiperCurActiveColor: 'rgba(0,0,0,.6)', // swiper轮播图选中圆点的颜色
    swiperCurColor: 'rgba(0,0,0,.2)', // swiper轮播图未选中圆点的颜色
 ...
 */
  ...
  */
  ...
  saveUserInfo (userInfo, cb) {
    // 请求后台登录接口--存入用户数据
    login(userInfo).then(({data: {code, message, data}}) => {
